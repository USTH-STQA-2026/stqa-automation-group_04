...
def test_login_fail_wrong_password(page, test_config): #Điệp#
    ...
    # [I] Infection: Nhập email đúng, mật khẩu sai
    flutter_fill(page, "Email", test_config["email"])
    flutter_fill(page, "Mật khẩu", "wrongpassword")
    flutter_click_button(page, "Đăng nhập")

    # [P] Propagation: Lỗi lan truyền ra giao diện
   ...
   # [R✓] Revealability
    ...
    has_error = "Mật khẩu không đúng" in sem_text
    has_user_name = test_config["display_name"] in sem_text
    has_logout = "Đăng xuất" in sem_text or "Logout" in sem_text

    assert has_error, \
     "Expected error message 'Mật khẩu không đúng' was not displayed"

    assert not (has_user_name or has_logout), \
     "Login unexpectedly succeeded with wrong password"
def test_login_fail_empty_fields(page, test_config): #Điệp03#
    ...
    # [I] Infection: Không nhập Email/Mật khẩu
    flutter_click_button(page, "Đăng nhập")
    ...
    has_error = "Vui lòng nhập email và mật khẩu" in sem_text
    has_user_name = test_config["display_name"] in sem_text
    has_logout = "Đăng xuất" in sem_text or "Logout" in sem_text

    assert has_error, \
        "Validation message 'Vui lòng nhập email và mật khẩu' was not displayed"

    assert not (has_user_name or has_logout), \
        "Login unexpectedly succeeded with empty credentials"